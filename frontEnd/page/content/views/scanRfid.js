...
async function submitScan() {
    ...
    statusEl.innerHTML = '<span class="badge bg-warning p-2 fs-6">Memproses...</span>';

    try {
        const response = await fetch('http://localhost:3000/api/attendances/store?card_id=' + encodeURIComponent(cardId) + '&mac_address=SCANGO-WEB', {
            method: 'POST',
        ...
        if (data.success) {
            statusEl.innerHTML = '<span class="badge bg-success p-2 fs-6">Berhasil!</span>';
            resultEl.innerHTML = '<div class="alert alert-success">' + (data.message || 'Kehadiran berhasil dicatat') + '</div>';
        } else {
            statusEl.innerHTML = '<span class="badge bg-danger p-2 fs-6">Gagal</span>';
            resultEl.innerHTML = '<div class="alert alert-danger">' + (data.error || data.message || 'Gagal mencatat kehadiran') + '</div>';
        }
    } catch (error) {
        statusEl.innerHTML = '<span class="badge bg-danger p-2 fs-6">Error</span>';
        resultEl.innerHTML = '<div class="alert alert-danger">Terjadi kesalahan koneksi ke server</div>';
    }
